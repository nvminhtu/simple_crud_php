...
<body>
	...
	<div id="catdlg" class="easyui-dialog" style="width:700px;height:600px;padding:10px 20px"
		...
		<form id="catfm" method="post" novalidate>
			...
			<div class="fitem">
				<label>Thumb:</label>
				<input name="thumb" class="easyui-textbox" style="width:100%;height:300px">
			</div>
		</form>
	</div>
	...
	<script type="text/javascript">
		...
		function editStory(){
			...
			if (row){
				...
				$('#fm').form('load',row);
				url = 'story/update_story.php?id='+row.id;
			}
		}
		function saveStory(){
			$('#fm').form('submit',{
		...
		function destroyStory(){
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to destroy this Story?',function(r){
					if (r){
						$.post('story/destroy_story.php',{id:row.id},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the Story data
							} else {
